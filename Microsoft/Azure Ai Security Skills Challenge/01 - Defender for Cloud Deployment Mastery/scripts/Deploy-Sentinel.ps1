...
# =============================================================================
# Orphaned Onboarding State Cleanup
# =============================================================================

Write-Host "🧹 Step 2A: Checking for orphaned onboarding states..." -ForegroundColor Cyan
...
try {
    ...
    if ($sentinelState) {
        Write-Host "🔍 Found existing onboarding state..." -ForegroundColor Yellow
        ...
        # Check if this is an orphaned state (null workspaceId indicates corruption)
        if (-not $sentinelState.properties.workspaceId -or $sentinelState.properties.workspaceId -eq $null) {
            Write-Host "⚠️ Detected orphaned onboarding state (null workspaceId)" -ForegroundColor Red
            Write-Host "   This can prevent data connectors from functioning properly" -ForegroundColor Yellow
            Write-Host "   Removing orphaned state and recreating..." -ForegroundColor Cyan
            
            if (-not $WhatIf) {
                try {
                    # Remove the orphaned onboarding state
                    Write-Host "   🗑️ Removing orphaned onboarding state..." -ForegroundColor White
                    Invoke-AzureRestApi -Method "DELETE" -Uri $sentinelUri
                    Write-Host "   ✅ Orphaned state removed successfully" -ForegroundColor Green
                    
                    # Wait a moment for the deletion to propagate
                    Start-Sleep -Seconds 5
                    Write-Host "   ⏳ Waiting for state cleanup to propagate..." -ForegroundColor White
                }
                catch {
                    Write-Host "   ⚠️ Could not remove orphaned state (may not exist): $($_.Exception.Message)" -ForegroundColor Yellow
                }
            }
        }
        else {
            Write-Host "✅ Existing onboarding state appears healthy" -ForegroundColor Green
            if (-not $WhatIf) {
    ...
    Write-Host "ℹ️ No existing onboarding state found - proceeding with fresh enablement" -ForegroundColor Yellow
}

# =============================================================================
# Current State Assessment (Post-Cleanup)
# =============================================================================

Write-Host "🔍 Step 2B: Verifying clean state for Sentinel enablement..." -ForegroundColor Cyan
...
try {
    Write-Host "   🚀 Creating fresh Sentinel onboarding state..." -ForegroundColor White
    ...
    if ($result.properties) {
        ...
        # Validate that the new state is properly configured
        Write-Host "   🔍 Validating new onboarding state..." -ForegroundColor White
        Start-Sleep -Seconds 3
        
        try {
            $validationState = Invoke-AzureRestApi -Method "GET" -Uri $sentinelUri
            if ($validationState.properties.workspaceId) {
                Write-Host "   ✅ Onboarding state validation successful" -ForegroundColor Green
                Write-Host "   📊 Workspace ID properly configured: $($validationState.properties.workspaceId)" -ForegroundColor Green
            }
            else {
                Write-Host "   ⚠️ Warning: Onboarding state may still have issues (workspaceId not set)" -ForegroundColor Yellow
            }
        }
        catch {
            Write-Host "   ⚠️ Could not validate new onboarding state: $($_.Exception.Message)" -ForegroundColor Yellow
        }
    }
    else {
