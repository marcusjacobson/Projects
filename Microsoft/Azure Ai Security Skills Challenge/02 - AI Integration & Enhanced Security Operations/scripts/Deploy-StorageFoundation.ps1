...
try {
    ...
    if ($WhatIf) {
        ...
        try {
            ...
            # Execute deployment using Azure CLI with inline parameters
            $DeployResult = az deployment sub create `
                --location $Location `
                --template-file $BicepFile `
                --parameters environmentName=$EnvironmentName `
                --parameters location="$Location" `
                --parameters storageBlobContributorAccount=$ObjectId `
                --parameters aiResourceGroupName=$ResourceGroupName `
                --name $DeploymentName `
    ...
    if ($WhatIf) {
        ...
        Write-Host "    - Soft delete disabled (blob & file services)" -ForegroundColor White
