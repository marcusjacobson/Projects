<#
    .SYNOPSIS
        Adds specified users as owners to a specified Entra group.

    .DESCRIPTION
        This script takes a JSON representation of users and an Entra group name as input, 
        verifies the existence of the group and users, and adds the users as owners to the specified group. 

    .PARAMETER UsersJson
        [string] A JSON string representing the users to be added as owners, based on their UPN (User Principal Name). This parameter is mandatory.

    .PARAMETER GroupName
        [string] The display name of the Entra group to which the users will be added as owners. This parameter is mandatory.
    ...
    .EXAMPLE
        .\Add-Owner-to-Group.ps1 -UsersJson '{"users":[{"userPrincipalName":"user1@domain.com"},{"userPrincipalName":"user2@domain.com"}]}' -GroupName "MyEntraGroup"
        This example adds the specified users as owners to the specified Entra group.

    .NOTES
        File Name      : Add-Owner-to-Group.ps1
...
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
Write-Verbose  "Adding owners to $GroupName" -Verbose
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
...
# Add users to the group as owners
if ($null -ne $users -and $users.Count -gt 0) {
    foreach ($user in $users) {
        ...
        if (-not [String]::IsNullOrEmpty($ownerResponse.error)) {
            if ($($ownerResponse.error.message) -eq "One or more added object references already exist for the following modified properties: 'owners'.") {
                Write-Verbose "        User $($user.id) is already an owner of group $($GroupName). Bypassing owner assignment to the group." -Verbose
                Write-Verbose "" -Verbose
                Continue
            }
            else {
            ...
            Write-Verbose "        User $($user.id) successfully added to group $($GroupName) as an owner." -Verbose
            Write-Verbose "" -Verbose
            $noOfOwnersAdded++
        }
    }
...
Write-Verbose  "-------------------------------------------------" -Verbose
Write-Verbose  "    Number of users added as owner of $GroupName       : $noOfOwnersAdded" -Verbose
Write-Verbose  "" -Verbose
