<#
    .SYNOPSIS
        Removes members from an Entra group based on a JSON input of user principal names (UPNs).
        
    .DESCRIPTION
        This script removes members from a specified Entra group by taking a JSON input of user principal names (UPNs). 
    ...
    .EXAMPLE
        .\Remove-Member-from-Group.ps1 -UsersJson '{"users":[{"userPrincipalName":"user1@domain.com"},{"userPrincipalName":"user2@domain.com"}]}' -GroupName "MyGroup"
        This example removes the users with UPNs user1@domain.com and user2@domain.com from the Entra group named "MyGroup".

    .NOTES
        File Name      : Remove-Member-from-Group.ps1
...
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
Write-Verbose  "Removing members from $GroupName" -Verbose
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose

# Initialize counters
$noOfMembersRemoved = 0
...
# Remove users from the group as members
if ($null -ne $users -and $users.Count -gt 0) {
    foreach ($user in $users) {
        ...
        $groupUri = "https://graph.microsoft.com/v1.0/groups/$groupId/members/$($user.id)/`$ref"
        ...
        # Check if user is not a member of the group. If the user is not a member, output a warning and continue to the next user, otherwise output confirmation the user was removed as a member.
        if (-not [String]::IsNullOrEmpty($memberResponse.error)) {
            if ($($memberResponse.error.code) -eq "Request_ResourceNotFound") {
                Write-Verbose "        User $($user.id) is not a member of group $($GroupName). Bypassing member removal from the group." -Verbose
                Write-Verbose "" -Verbose
                Continue
            }
            else {
                Write-Error "           Failed to remove user $($user.id) from group $($GroupName) because of [$($memberResponse.error.code) - $($memberResponse.error.message)]." -Verbose
                $global:ScriptFailed = $true # Set the global ScriptFailed variable to true, to only exit with an error code if there is a failure other than the user not being a member of the group
            }
        }
        else {
...
Write-Verbose  "-------------------------------------------------" -Verbose
Write-Verbose  "    Number of users removed as members of $GroupName       : $noOfMembersRemoved" -Verbose
Write-Verbose  "" -Verbose
