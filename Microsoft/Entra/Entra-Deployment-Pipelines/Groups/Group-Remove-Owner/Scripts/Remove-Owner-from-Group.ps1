<#
    .SYNOPSIS
        Removes owners from an Entra group based on a JSON input of user principal names (UPNs).
        
    .DESCRIPTION
        This script removes owners from a specified Entra group by taking a JSON input of user principal names (UPNs). 
        It verifies the existence of the group and users in Entra, and then removes the users from the group. 

    .PARAMETER UsersJson
        JSON representation of users to remove based on UPN. This parameter is mandatory and must be a valid JSON string.

    .PARAMETER GroupName
        The name of the Entra group to remove the users from. This parameter is mandatory, and the script will confirm it is a valid group name.

    .INPUTS
        The script is designed to be run from Azure Pipelines, with variable inputs provided by pipeline-variables.yml.

    .OUTPUTS
        Verbose output indicating the status of the group and user verification, and the result of the owner addition process.

    .EXAMPLE
        .\Remove-owner-from-Group.ps1 -UsersJson '{"users":[{"userPrincipalName":"user1@domain.com"},{"userPrincipalName":"user2@domain.com"}]}' -GroupName "MyGroup"
        This example removes the users with UPNs user1@domain.com and user2@domain.com from the Entra group named "MyGroup".

    .NOTES
        File Name      : Remove-owner-from-Group.ps1
        Author         : Marcus Jacobson
        Version History: 1.0.0, Initial version
        Release        : 2025-03-06 - this is the initial release date
        Updated        : 
#>
...
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
Write-Verbose  "Removing owners from $GroupName" -Verbose
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
...
# Remove users from the group as owners
if ($null -ne $users -and $users.Count -gt 0) {
    foreach ($user in $users) {
        ...
        $groupUri = "https://graph.microsoft.com/v1.0/groups/$groupId/owners/$($user.id)/`$ref"
        ...
        # Check if user is not an owner of the group. If the user is not an owner, output a warning and continue to the next user, otherwise output confirmation the user was removed as an owner.
        if (-not [String]::IsNullOrEmpty($ownerResponse.error)) {
            if ($($ownerResponse.error.code) -eq "Request_ResourceNotFound") {
                Write-Verbose "        User $($user.id) is not an owner of group $($GroupName). Bypassing owner removal from the group." -Verbose
                Write-Verbose "" -Verbose
                Continue
            }
            else {
                Write-Error "           Failed to remove user $($user.id) from group $($GroupName) because of [$($ownerResponse.error.code) - $($ownerResponse.error.message)]." -Verbose
                $global:ScriptFailed = $true # Set the global ScriptFailed variable to true, to only exit with an error code if there is a failure other than the user not being a owner of the group
            }
        }
        else {
            Write-Verbose "        User $($user.id) successfully removed from group $($GroupName)." -Verbose
            Write-Verbose "" -Verbose
            $noOfOwnersRemoved++
        }
    }
...
Write-Verbose  "-------------------------------------------------" -Verbose
Write-Verbose  "    Number of users removed as owners of $GroupName       : $noOfOwnersRemoved" -Verbose
Write-Verbose  "" -Verbose
