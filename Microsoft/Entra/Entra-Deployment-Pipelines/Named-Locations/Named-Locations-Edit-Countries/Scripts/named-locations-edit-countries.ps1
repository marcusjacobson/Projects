<#
    .SYNOPSIS
        This script updates a named location list in Entra Conditional Access with a specified list of countries and regions.
    ...
    .DESCRIPTION
        ...
        abbreviations, and then edits the named location list based on the provided attributes. 

    .PARAMETER CountriesJson
        [string] A JSON string representing the countries and regions to be included in the named location list. This parameter is mandatory.

    .PARAMETER DisplayName
        [string] The display name for the named location list. This parameter is mandatory.

    .PARAMETER IncludeUnknownCountriesAndRegions
        [string] A boolean flag indicating whether to include unknown countries and regions. This parameter is mandatory.
    ...
    .EXAMPLE
        .\named-locations-edit-countries.ps1 -CountriesJson '{"countriesAndRegions":[{"countryOrRegion":"US"},{"countryOrRegion":"CA"}]}' -DisplayName "North America" -IncludeUnknownCountriesAndRegions $false
        This example updates the named location list "North America" to include the United States and Canada, without including unknown countries and regions.

    .NOTES
        File Name      : named-locations-edit-countries.ps1
...
# Create/update users
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
Write-Verbose  "Updating named location with valid countries" -Verbose
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
...
if (-not [String]::IsNullOrEmpty($response.error)) {
    ...
    Write-Verbose "    The named location list $($DisplayName) was successfully updated to include the following countries:" -Verbose
