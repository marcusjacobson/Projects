<#
    .SYNOPSIS
        Adds members to an Entra PIM group based on a JSON input of user principal names (UPNs).

    .DESCRIPTION
        This script is designed to add members to an Entra PIM group by taking a JSON input of user principal names (UPNs) and the group name. 
    ...
    .EXAMPLE
        .\Add-Member-to-PIM-Group.ps1 -UsersJson '{"users":[{"userPrincipalName":"user1@domain.com"},{"userPrincipalName":"user2@domain.com"}]}' -GroupName "MyEntraGroup"
        This example adds the users with the specified UPNs to the Entra group "MyEntraGroup".
    .NOTES
        File Name      : Add-Member-to-PIM-Group.ps1
...
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
Write-Verbose  "Adding PIM memberships to $GroupName" -Verbose
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
...
# Add users to the PIM group as members
if ($null -ne $users -and $users.Count -gt 0) {
    foreach ($user in $users) {
        ...
        if (-not [String]::IsNullOrEmpty($memberResponse.error)) {
            if ($($memberResponse.error.message) -eq "The Role assignment already exists.") {
                Write-Verbose "        User $($user.id) is already a member of PIM group $($GroupName). Bypassing member assignment to the group." -Verbose
                Write-Verbose "" -Verbose
                Continue
            }
            elseif ($($memberResponse.error.message) -eq "Resource type not supported for onboarding") {
            ...
            Write-Verbose "        User $($user.id) successfully added to group $($GroupName) as a member." -Verbose
            Write-Verbose "" -Verbose
            $noOfMembersAdded++
        }
    }
...
Write-Verbose  "-------------------------------------------------" -Verbose
Write-Verbose  "    Number of users added to the PIM Group $GroupName as members       : $noOfMembersAdded" -Verbose
Write-Verbose  "" -Verbose
