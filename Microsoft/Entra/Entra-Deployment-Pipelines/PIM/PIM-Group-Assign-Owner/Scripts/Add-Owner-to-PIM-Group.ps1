<#
    .SYNOPSIS
        Adds owners to an Entra PIM group based on a JSON input of user principal names (UPNs).

    .DESCRIPTION
        This script is designed to add owners to an Entra PIM group by taking a JSON input of user principal names (UPNs) and the group name. 
        It verifies the existence of the group and users in Entra, and then assigns the users to the specified PIM group. 

    .PARAMETER UsersJson
        [string] A JSON representation of users to add based on UPN. This parameter is mandatory.

    .PARAMETER GroupName
        [string] The name of the Entra group to which the users will be added. This parameter is mandatory.
    ...
    .EXAMPLE
        .\Add-Owner-to-PIM-Group.ps1 -UsersJson '{"users":[{"userPrincipalName":"user1@domain.com"},{"userPrincipalName":"user2@domain.com"}]}' -GroupName "MyEntraGroup"
        This example adds the users with the specified UPNs to the Entra group "MyEntraGroup".
    .NOTES
        File Name      : Add-Owner-to-PIM-Group.ps1
...
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
Write-Verbose  "Adding PIM Owner Assignments to $GroupName" -Verbose
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
...
# Add users to the PIM group as owners
if ($null -ne $users -and $users.Count -gt 0) {
    foreach ($user in $users) {
        ...
        if (-not [String]::IsNullOrEmpty($ownerResponse.error)) {
            if ($($ownerResponse.error.message) -eq "The Role assignment already exists.") {
                Write-Verbose "        User $($user.id) is already an owner of PIM group $($GroupName). Bypassing owner assignment to the group." -Verbose
                Write-Verbose "" -Verbose
                Continue
            }
            ...
            Write-Verbose "        User $($user.id) successfully added to PIM group $($GroupName) as an owner." -Verbose
            Write-Verbose "" -Verbose
            $noOfOwnersAdded++
        }
    }
...
Write-Verbose  "-------------------------------------------------" -Verbose
Write-Verbose  "    Number of users added to the PIM Group $GroupName as owners       : $noOfOwnersAdded" -Verbose
Write-Verbose  "" -Verbose
