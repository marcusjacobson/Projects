<#
    .SYNOPSIS
        Extends members to an Entra PIM group for one year, based on a JSON input of user principal names (UPNs).

    .DESCRIPTION
        This script is designed to extend members to an Entra PIM group by taking a JSON input of user principal names (UPNs) and the group name. 
        It verifies the existence of the group and users in Entra, and then extends the users' access to the specified PIM group. 

    .PARAMETER UsersJson
        [string] A JSON representation of users to extend based on UPN. This parameter is mandatory.

    .PARAMETER GroupName
        [string] The name of the Entra group to which the users will be extended. This parameter is mandatory.
    ...
    .EXAMPLE
        .\Extend-Member-for-PIM-Group.ps1 -UsersJson '{"users":[{"userPrincipalName":"user1@domain.com"},{"userPrincipalName":"user2@domain.com"}]}' -GroupName "MyEntraGroup"
        This example extends the users with the specified UPNs to the Entra group "MyEntraGroup".

    .NOTES
        File Name      : Extend-Member-for-PIM-Group.ps1
...
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
Write-Verbose  "Extending PIM memberships for $GroupName" -Verbose
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
...
foreach ($userObject in $userObjects.users) {
    ...
    elseif ($existingUser.value -and $existingUser.value.Count -gt 0) {
            ...
            Write-Warning ('            Bypassing user [{0}] from role extension for the group because it does not exist.' -f $userObject)
            Write-Verbose "" -Verbose
            continue
    }
}

# Extending members of the PIM group
if ($null -ne $users -and $users.Count -gt 0) {
    foreach ($user in $users) {
        ...
        # Define the API body for the assignment, extending the eligibility for one year from the date the pipeline is run.
        ...
        if (-not [String]::IsNullOrEmpty($memberResponse.error)) {
            if ($($memberResponse.error.message) -eq "Resource type not supported for onboarding") {
                ...
                Write-Error "           Failed to extend user $($user.id) of the group $($GroupName) because of [$($memberResponse.error.code) - $($memberResponse.error.message)]." -Verbose
                $global:ScriptFailed = $true # Set the global ScriptFailed variable to true, to only exit with an error code if there is a failure other than the user already being a member of the group
            }
        }
        else {
            Write-Verbose "        User $($user.id) successfully extended for PIM group $($GroupName) as a member." -Verbose
            Write-Verbose "" -Verbose
            $noOfMembersAdded++
        }
    }
...
Write-Verbose  "-------------------------------------------------" -Verbose
Write-Verbose  "    Number of users extended as members of the PIM Group $GroupName       : $noOfMembersAdded" -Verbose
Write-Verbose  "" -Verbose
