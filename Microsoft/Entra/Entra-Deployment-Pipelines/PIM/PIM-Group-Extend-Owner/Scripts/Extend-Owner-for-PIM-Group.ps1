<#
    .SYNOPSIS
        Extends owners of an Entra PIM group for one year, based on a JSON input of user principal names (UPNs).

    .DESCRIPTION
        This script is designed to extend owners to an Entra PIM group by taking a JSON input of user principal names (UPNs) and the group name. 
    ...
    .EXAMPLE
        .\Extend-Owner-for-PIM-Group.ps1 -UsersJson '{"users":[{"userPrincipalName":"user1@domain.com"},{"userPrincipalName":"user2@domain.com"}]}' -GroupName "MyEntraGroup"
        This example extends the users with the specified UPNs to the Entra group "MyEntraGroup".

    .NOTES
        File Name      : Extend-Owner-for-PIM-Group.ps1
...
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
Write-Verbose  "Extending PIM ownerships for $GroupName" -Verbose
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
...
# Extend owners of the PIM group
if ($null -ne $users -and $users.Count -gt 0) {
    foreach ($user in $users) {
        ...
        if (-not [String]::IsNullOrEmpty($ownerResponse.error)) {
            if ($($ownerResponse.error.message) -eq "Resource type not supported for onboarding") {
                ...
                Write-Error "           Failed to add user $($user.id) to group $($GroupName) because of [$($ownerResponse.error.code) - $($ownerResponse.error.message)]." -Verbose
                $global:ScriptFailed = $true # Set the global ScriptFailed variable to true, to only exit with an error code if there is a failure other than the user already being an owner of the group
            }
        }
        else {
            Write-Verbose "        User $($user.id) successfully extended for PIM group $($GroupName) as an owner." -Verbose
            Write-Verbose "" -Verbose
            $noOfOwnersAdded++
        }
    }
...
Write-Verbose  "-------------------------------------------------" -Verbose
Write-Verbose  "    Number of users extended as owners of the PIM Group $GroupName       : $noOfOwnersAdded" -Verbose
Write-Verbose  "" -Verbose
