<#
    .SYNOPSIS
        Removes members from an Entra PIM group, based on a JSON input of user principal names (UPNs).

    .DESCRIPTION
        This script is designed to remove members from an Entra PIM group by taking a JSON input of user principal names (UPNs) and the group name. 
    ...
    .EXAMPLE
        .\Remove-Member-from-PIM-Group.ps1 -UsersJson '{"users":[{"userPrincipalName":"user1@domain.com"},{"userPrincipalName":"user2@domain.com"}]}' -GroupName "MyEntraGroup"
        This example removes the users with the specified UPNs from the Entra group "MyEntraGroup".

    .NOTES
        File Name      : Remove-Member-from-PIM-Group.ps1
...
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
Write-Verbose  "Removing PIM memberships from $GroupName" -Verbose
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
...
# Remove users from the PIM group as members
if ($null -ne $users -and $users.Count -gt 0) {
    foreach ($user in $users) {
        ...
        if (-not [String]::IsNullOrEmpty($memberResponse.error)) {
            if ($($memberResponse.error.message) -eq "The Role assignment does not exist.") {
                Write-Warning "           User $($user.id) is not an existing member of the group $($GroupName). Skipping user." -Verbose
                Write-Verbose "" -Verbose
            }
            elseif ($($memberResponse.error.message) -eq "Resource type not supported for onboarding") {
                ...
                Write-Error "           Failed to remove user $($user.id) from the group $($GroupName) because of [$($memberResponse.error.code) - $($memberResponse.error.message)]." -Verbose
                $global:ScriptFailed = $true # Set the global ScriptFailed variable to true, to only exit with an error code if there is a failure other than the user already being a member of the group
            }
        }
        else {
            Write-Verbose "        User $($user.id) successfully removed from PIM group $($GroupName) as a member." -Verbose
            Write-Verbose "" -Verbose
            $noOfMembersRemoved++
        }
    }
...
Write-Verbose  "-------------------------------------------------" -Verbose
Write-Verbose  "    Number of users removed from the PIM Group $GroupName as members       : $noOfMembersRemoved" -Verbose
Write-Verbose  "" -Verbose
