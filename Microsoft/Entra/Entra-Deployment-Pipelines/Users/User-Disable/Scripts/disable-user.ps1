<#
    .SYNOPSIS
        Disables user in Entra based on the provided UserPrincipalName and JSON input.

    .DESCRIPTION
        This script disables a user in Entra by taking a UserPrincipalName and a JSON string representing the user properties to be updated.
        It verifies the existence of the user, and then disables them using the Microsoft Graph API. 
    ...
    .EXAMPLE
        .\disable-user.ps1 -UserPrincipalName "user@example.com" -UserJson '{"accountEnabled": false}'
        This example disables the user with the UPN "user@example.com".

    .NOTES
        File Name      : disable-user.ps1
...
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
Write-Verbose  "Disabling $UserPrincipalName" -Verbose
Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
...
# Ensures that only the accountEnabled property is passed into the script
$validProperties = @(
    "accountEnabled"
)
...
foreach ($property in $userProperties.PSObject.Properties) {
    ...
    # Check if the property is a valid Entra user property - for this script, the only valid property is accountEnabled
    if ($validProperties -notcontains $propertyName) {
        Write-Error "    Property '$propertyName' is not a valid Entra user property. For this script, accountEnabled is the only valid property. Exiting pipeline." -Verbose
        ...
        Write-Verbose "    The user $($UserPrincipalName) has been disabled." -Verbose
