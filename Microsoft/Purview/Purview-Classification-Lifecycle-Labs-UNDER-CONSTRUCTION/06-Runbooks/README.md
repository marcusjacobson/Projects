...
- Reference SharePoint test site setup for incident reproduction.
- Leverage placeholder SIT configurations for troubleshooting.
- Validate initial retention label configurations during health checks.

### Labs 1-2: Custom SITs (Regex & EDM)

- Validate SIT definitions during health checks (Lab 1 regex patterns, Lab 2 EDM schemas).
- Troubleshoot pattern matching issues using Lab 1 regex testing techniques.
- Backup and restore SIT configurations (both regex and EDM types).
- Reference EDM upload procedures from Lab 2 for data refresh operations.

### Lab 3: On-Demand Classification & Content Explorer

- Use On-Demand Classification scripts for health check validations.
- Reference classification logs and Content Explorer for troubleshooting.
...
- Troubleshoot label policy issues and simulation mode behaviors.
- Validate label coverage metrics and auto-apply policy effectiveness.

### Lab 5: PowerShell Automation & Scaling
