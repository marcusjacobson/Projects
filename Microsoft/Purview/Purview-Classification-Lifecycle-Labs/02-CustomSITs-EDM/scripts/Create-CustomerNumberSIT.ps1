<#
.SYNOPSIS
    Creates the "Contoso Customer Number" custom Sensitive Information Type in Microsoft Purview.

.DESCRIPTION
    This script creates a custom SIT for detecting organization-specific customer account numbers
    following the pattern CUST-###### (e.g., CUST-100234).
    
    The custom SIT includes:
    - Regex pattern matching: \bCUST-\d{6}\b
    - Context keywords: customer, account, CUST, client, consumer
    ...
    detection for customer identification numbers with context-aware confidence scoring.
...
.EXAMPLE
    ...
    Creates the "Contoso Customer Number" custom SIT with default settings.

.EXAMPLE
    .\Create-CustomerNumberSIT.ps1 -Confirm
...
.NOTES
    ...
    Pattern Design:
    - Format: CUST-###### (6-digit customer identifier)
    - Word boundaries (\b) prevent partial matches
    - Case-insensitive matching for flexibility
    - 6-digit requirement provides adequate uniqueness
    
    Confidence Levels:
    - High (85%): Multiple keywords indicate strong customer context
    - Medium (75%): Single keyword suggests customer relationship
    - Low (65%): Pattern-only for basic customer number detection
    
    Script development orchestrated using GitHub Copilot.

.SIT CONFIGURATION
    - Name: Contoso Customer Number
    - Pattern: CUST-###### (e.g., CUST-100234, CUST-102089)
    - Keywords: customer, account, CUST, client, consumer, subscriber
    - Character Proximity: 300 characters (for keyword matching)
    - Instance Count: 1 minimum (single occurrence detection)
#>
#
# =============================================================================
# Create "Contoso Customer Number" custom SIT for Microsoft Purview
# =============================================================================
...
try {
    ...
    # Define regex pattern for Customer Number: CUST-######
    $regexPattern = '\bCUST-\d{6}\b'
    ...
    Write-Host "   • Format: CUST-###### (6-digit customer identifier)" -ForegroundColor White
    Write-Host "   • Example Matches: CUST-100234, CUST-102089, CUST-999999" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "   • Purpose: Enhance confidence when customer context is present" -ForegroundColor White
    Write-Host ""
  ...
  <RulePack id="$(New-Guid)">
    ...
    <Details defaultLangCode="en-us">
      <LocalizedDetails langcode="en-us">
        ...
        <Name>Contoso Customer Number SIT</Name>
        <Description>Custom SIT for detecting Contoso customer account numbers in format CUST-######</Description>
      </LocalizedDetails>
    </Details>
  </RulePack>
  <Rules>
    <Entity id="$(New-Guid)" patternsProximity="300" recommendedConfidence="75">
      <Pattern confidenceLevel="85">
        ...
        <Any minMatches="2">
          <Match idRef="Keyword_CustomerNumber"/>
        </Any>
      </Pattern>
      ...
      <Resource idRef="Regex_CustomerNumber">
        <Regex id="Regex_CustomerNumber">$regexPattern</Regex>
      </Resource>
      <Resource idRef="Keyword_CustomerNumber">
        <Keywords>
...
"@
    ...
    New-DlpSensitiveInformationType `
        -Name $sitName `
        -Description "Custom SIT for detecting Contoso customer account numbers (CUST-######). Includes context keywords for confidence scoring." `
...
Write-Host "   • Format: CUST-###### (e.g., CUST-100234)" -ForegroundColor White
...
Write-Host "      - Search for 'Contoso Customer Number'" -ForegroundColor White
Write-Host ""
Write-Host "   3. Create remaining custom SIT:" -ForegroundColor White
Write-Host "      - Run .\Create-PurchaseOrderSIT.ps1" -ForegroundColor White
Write-Host ""
Write-Host "   4. Test all custom SITs together:" -ForegroundColor White
...
Write-Host "   - Verify customer number format matches your organization's actual pattern" -ForegroundColor White
Write-Host "   - Test with real customer data samples before production deployment" -ForegroundColor White
Write-Host "   - Monitor false positive rate for 6-digit number patterns" -ForegroundColor White
Write-Host "   - Consider adding organization-specific keywords if needed" -ForegroundColor White
Write-Host "   - Use in combination with DLP policies for customer data protection" -ForegroundColor White
Write-Host ""
