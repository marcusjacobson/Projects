<#
.SYNOPSIS
    Creates the "Contoso Purchase Order Number" custom Sensitive Information Type in Microsoft Purview.

.DESCRIPTION
    This script creates a custom SIT for detecting organization-specific purchase order numbers
    following the pattern PO-####-####-XXXX (e.g., PO-3200-2025-DELL).
    
    The custom SIT includes:
    - Regex pattern matching: \bPO-\d{4}-\d{4}-[A-Z]{4}\b
    - Context keywords: purchase order, PO, procurement, requisition, vendor
    ...
    Pattern Components:
    - Department code (####): 4-digit department identifier
    - Year (####): 4-digit year (e.g., 2025)
    - Vendor code (XXXX): 4-letter vendor abbreviation (uppercase)
    
    This SIT is designed for Lab 2 (Custom SITs) to demonstrate complex regex-based pattern
    detection for procurement document tracking with multi-segment identifiers.
...
.EXAMPLE
    ...
    Creates the "Contoso Purchase Order Number" custom SIT with default settings.

.EXAMPLE
    .\Create-PurchaseOrderSIT.ps1 -Confirm
...
.NOTES
    ...
    Pattern Design:
    - Format: PO-####-####-XXXX (dept-year-vendor)
    - Example: PO-3200-2025-DELL (IT dept, year 2025, Dell vendor)
    - Word boundaries (\b) prevent partial matches
    - Case-sensitive for vendor code consistency
    - Complex multi-segment structure for uniqueness
    
    Confidence Levels:
    - High (85%): Multiple keywords indicate strong procurement context
    - Medium (75%): Single keyword suggests purchase order workflow
    - Low (65%): Pattern-only for basic PO number detection
    
    Use Cases:
    - Procurement document classification
    - Purchase order tracking across departments
...
.SIT CONFIGURATION
    - Name: Contoso Purchase Order Number
    - Pattern: PO-####-####-XXXX (e.g., PO-3200-2025-DELL, PO-4100-2025-GOOG)
    - Keywords: purchase order, PO, procurement, requisition, vendor, supplier, order
    - Character Proximity: 300 characters (for keyword matching)
    - Instance Count: 1 minimum (single occurrence detection)
#>
#
# =============================================================================
# Create "Contoso Purchase Order Number" custom SIT for Microsoft Purview
# =============================================================================
...
try {
    ...
    # Define regex pattern for Purchase Order: PO-####-####-XXXX
    $regexPattern = '\bPO-\d{4}-\d{4}-[A-Z]{4}\b'
    ...
    Write-Host "   • Format: PO-####-####-XXXX (dept-year-vendor)" -ForegroundColor White
    Write-Host "   • Example Matches:" -ForegroundColor White
    Write-Host "     - PO-3200-2025-DELL (IT dept, 2025, Dell)" -ForegroundColor White
    Write-Host "     - PO-4100-2025-GOOG (Marketing, 2025, Google)" -ForegroundColor White
    Write-Host "     - PO-5000-2024-MSFT (Finance, 2024, Microsoft)" -ForegroundColor White
    Write-Host ""
    
    Write-Host "   Pattern Components:" -ForegroundColor Cyan
    Write-Host "   • Department Code: #### (4-digit identifier)" -ForegroundColor White
    Write-Host "   • Year: #### (4-digit year, typically 2024-2026)" -ForegroundColor White
    Write-Host "   • Vendor Code: XXXX (4-letter uppercase abbreviation)" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "   • Purpose: Enhance confidence in procurement document context" -ForegroundColor White
    Write-Host ""
  ...
  <RulePack id="$(New-Guid)">
    ...
    <Details defaultLangCode="en-us">
      <LocalizedDetails langcode="en-us">
        ...
        <Name>Contoso Purchase Order Number SIT</Name>
        <Description>Custom SIT for detecting Contoso purchase order numbers in format PO-####-####-XXXX (dept-year-vendor)</Description>
      </LocalizedDetails>
    </Details>
  </RulePack>
  <Rules>
    <Entity id="$(New-Guid)" patternsProximity="300" recommendedConfidence="75">
      <Pattern confidenceLevel="85">
        ...
        <Any minMatches="2">
          <Match idRef="Keyword_PurchaseOrder"/>
        </Any>
      </Pattern>
      ...
      <Resource idRef="Regex_PurchaseOrder">
        <Regex id="Regex_PurchaseOrder">$regexPattern</Regex>
      </Resource>
      <Resource idRef="Keyword_PurchaseOrder">
        <Keywords>
...
"@
    ...
    New-DlpSensitiveInformationType `
        -Name $sitName `
        -Description "Custom SIT for detecting Contoso purchase order numbers (PO-####-####-XXXX). Tracks department, year, and vendor codes. Includes context keywords for confidence scoring." `
...
Write-Host "   • Format: PO-####-####-XXXX (e.g., PO-3200-2025-DELL)" -ForegroundColor White
Write-Host "   • Pattern Components: Department (4 digits), Year (4 digits), Vendor (4 uppercase letters)" -ForegroundColor White
...
Write-Host ""
Write-Host "   2. Verify all three custom SITs in Purview Compliance Portal:" -ForegroundColor White
...
Write-Host "      - Search for 'Contoso' to see all three custom SITs:" -ForegroundColor White
...
Write-Host ""
Write-Host "   3. Test all custom SITs together:" -ForegroundColor White
Write-Host "      - Upload Lab 2 test files to SharePoint site" -ForegroundColor White
Write-Host "      - Run On-Demand Classification scan on the site" -ForegroundColor White
Write-Host "      - Validate detection results in Content Explorer" -ForegroundColor White
Write-Host ""
...
Write-Host "      - Review detection accuracy for all three patterns" -ForegroundColor White
Write-Host "      - Check confidence level distribution" -ForegroundColor White
Write-Host ""
Write-Host "   5. Proceed to EDM configuration (Lab 2 Phase 2):" -ForegroundColor White
Write-Host "      - .\Create-EDMSourceDatabase.ps1" -ForegroundColor White
Write-Host "      - Follow EDM workflow for exact data match configuration" -ForegroundColor White
Write-Host ""

Write-Host "💡 Production Tips:" -ForegroundColor Yellow
Write-Host "   - Verify PO number format matches your organization's procurement system" -ForegroundColor White
Write-Host "   - Test with actual purchase order documents before production" -ForegroundColor White
Write-Host "   - Complex patterns like PO-####-####-XXXX have lower false positive rates" -ForegroundColor White
Write-Host "   - Consider department-specific keywords for higher confidence detection" -ForegroundColor White
Write-Host "   - Use in DLP policies to protect procurement and financial documents" -ForegroundColor White
Write-Host "   - Monitor Activity Explorer for classification trends across departments" -ForegroundColor White
Write-Host ""

Write-Host "✅ Script execution completed successfully" -ForegroundColor Green
Write-Host ""
Write-Host "🎯 All three regex-based custom SITs are now created:" -ForegroundColor Green
Write-Host "   ✅ Contoso Project Identifier (PROJ-YYYY-####)" -ForegroundColor Green
Write-Host "   ✅ Contoso Customer Number (CUST-######)" -ForegroundColor Green
Write-Host "   ✅ Contoso Purchase Order Number (PO-####-####-XXXX)" -ForegroundColor Green
