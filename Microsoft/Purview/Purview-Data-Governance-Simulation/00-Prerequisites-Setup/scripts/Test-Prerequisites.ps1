<#
.SYNOPSIS
    Validates environment prerequisites for Purview Data Governance Simulation.

.DESCRIPTION
    This script performs comprehensive validation of the local environment to ensure all
    requirements are met before beginning the Purview Data Governance Simulation project.
...
if (-not $SkipConnectivityTests) {
    ...
    Write-Host "   📋 Please sign in with SharePoint Administrator credentials" -ForegroundColor Cyan
    
    try {
        # Connect to SharePoint first (required)
        ...
        # Try Compliance connection (optional - may fail due to WAM issues)
        Write-Host "   📋 Attempting Compliance Center connection (optional)..." -ForegroundColor Cyan
        try {
            Connect-PurviewServices -Services @("ComplianceCenter") -TenantUrl $config.Environment.TenantUrl -PnPClientId $config.Environment.PnPClientId -Interactive
            
            $complianceTest = Test-PurviewConnection -Service "ComplianceCenter"
            if ($complianceTest.Connected) {
                Write-Host "   ✅ Security & Compliance: $($complianceTest.Message)" -ForegroundColor Green
            } else {
                Write-Host "   ⚠️  Security & Compliance: Connection test inconclusive" -ForegroundColor Yellow
            }
        } catch {
            Write-Host "   ⚠️  Security & Compliance: Could not connect (this is optional for prerequisites)" -ForegroundColor Yellow
            Write-Host "   💡 You can connect to Compliance Center later when needed for specific labs" -ForegroundColor Cyan
        }
...
if ($validationErrors.Count -eq 0) {
    ...
    Write-Host "   ✅ Environment ready for Purview Data Governance Simulation" -ForegroundColor Green
    Write-Host ""
