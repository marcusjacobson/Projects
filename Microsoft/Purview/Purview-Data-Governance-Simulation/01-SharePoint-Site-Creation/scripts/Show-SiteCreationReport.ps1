...
Write-Host "🔍 Step 1: Locate Most Recent Site Creation Report" -ForegroundColor Green
...
if (-not (Test-Path $reportsPath)) {
    ...
    Write-Host "   💡 Tip: Run New-SimulatedSharePointSites.ps1 first to generate a report" -ForegroundColor Yellow
    exit 1
}
...
if (-not $latestReport) {
    Write-Host "   ❌ No site creation reports found" -ForegroundColor Red
    Write-Host "   💡 Tip: Run New-SimulatedSharePointSites.ps1 first to generate a report" -ForegroundColor Yellow
    exit 1
}

Write-Host "   ✅ Found report: $($latestReport.Name)" -ForegroundColor Green
Write-Host "   📅 Created: $($latestReport.LastWriteTime.ToString('yyyy-MM-dd HH:mm:ss'))" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "🔍 Step 2: Load Report Data" -ForegroundColor Green
Write-Host "============================" -ForegroundColor Green
...
Write-Host ""
Write-Host "📊 Site Creation Report Summary" -ForegroundColor Cyan
...
if ($reportData.SkippedSites.Count -gt 0) {
    Write-Host ""
    Write-Host "⏭️  Skipped Sites:" -ForegroundColor Yellow
...
if ($reportData.SitesFailed -gt 0) {
    Write-Host "⚠️  Some sites failed to create. Review errors and retry if needed." -ForegroundColor Yellow
    exit 1
} else {
    Write-Host "✅ Report displayed successfully" -ForegroundColor Green
    exit 0
}
