...
.DESCRIPTION
    This script creates realistic financial documents for the Purview Data Governance Simulation
    containing credit card numbers, bank account numbers, ABA routing numbers, and IBAN patterns
    ...
    - Wire transfer documents with IBAN
    ...
    - IBAN: International banking account numbers
...
function New-SimulatedIBAN {
    $countryCodes = @("GB", "DE", "FR", "IT", "ES")
    $country = $countryCodes[(Get-Random -Minimum 0 -Maximum $countryCodes.Count)]
    $checkDigits = "{0:00}" -f (Get-Random -Minimum 10 -Maximum 99)
    
    $accountPart = ""
    for ($i = 0; $i -lt 18; $i++) {
        if ($i -lt 4) {
            $accountPart += [char](Get-Random -Minimum 65 -Maximum 91)
        } else {
            $accountPart += (Get-Random -Minimum 0 -Maximum 10)
        }
    }
    
    return "$country$checkDigits$accountPart"
}
...
$documentTypes = @(
    @{ Type = "ExpenseReport"; Template = "Expense Report - {0}"; CCCount = 8; BankCount = 0; RoutingCount = 0; IBANCount = 0 }
    @{ Type = "PaymentVoucher"; Template = "Payment Voucher - {0}"; CCCount = 2; BankCount = 4; RoutingCount = 4; IBANCount = 0 }
    @{ Type = "BankStatement"; Template = "Bank Statement - {0}"; CCCount = 0; BankCount = 6; RoutingCount = 2; IBANCount = 0 }
    @{ Type = "CreditCardTransaction"; Template = "CC Transaction Log - {0}"; CCCount = 15; BankCount = 0; RoutingCount = 0; IBANCount = 0 }
    @{ Type = "InvoicePayment"; Template = "Invoice Payment - {0}"; CCCount = 1; BankCount = 3; RoutingCount = 3; IBANCount = 0 }
    @{ Type = "WireTransfer"; Template = "Wire Transfer - {0}"; CCCount = 0; BankCount = 2; RoutingCount = 2; IBANCount = 4 }
    @{ Type = "ACHAuthorization"; Template = "ACH Payment Auth - {0}"; CCCount = 0; BankCount = 5; RoutingCount = 5; IBANCount = 0 }
)
...
"@
    ...
    # Add IBAN numbers
    for ($j = 0; $j -lt $docType.IBANCount; $j++) {
        $iban = New-SimulatedIBAN
        $content += "`nIBAN: $iban"
        $ibanCount++
    }
...
Write-Host "   📊 IBAN Numbers: $ibanCount" -ForegroundColor Cyan
