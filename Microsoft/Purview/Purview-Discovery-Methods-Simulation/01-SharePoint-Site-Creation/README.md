...
This lab creates and configures simulated SharePoint Online sites that will host test content for the Purview Discovery Methods Simulation. Using a 4-script workflow, you will provision sites from configuration, apply permissions and metadata, validate the complete setup, and generate comprehensive reports.
...
```powershell
cd "c:\REPO\GitHub\Projects\Microsoft\Purview\Purview-Discovery-Methods-Simulation\01-SharePoint-Site-Creation"
...
```powershell
# Option 1: Use -SkipExisting parameter (Recommended for partial runs)
.\scripts\New-SimulatedSharePointSites.ps1 -SkipExisting
```

```powershell
# Option 2: Full cleanup and recreate (Recommended for alias conflicts)
# This removes sites, M365 groups, and clears Azure AD alias reservations
..\06-Cleanup-Reset\scripts\Remove-SimulationResources.ps1 -ResourceType "Sites" -Force

# Wait 30 seconds, then recreate
.\scripts\New-SimulatedSharePointSites.ps1 -SkipExisting
```

> **💡 Tip**: If sites are created with numeric suffixes (e.g., `HR-Simulation2`), this indicates alias reservations from deleted groups. Use Option 2 for complete cleanup.
...
- The script includes automatic cleanup-and-retry logic that removes partially created resources and retries
- If a site times out repeatedly, run Lab 06 cleanup for that site, then retry:

  ```powershell
  # Full cleanup of stuck site (from Lab 06)
  ..\06-Cleanup-Reset\scripts\Remove-SimulationResources.ps1 -ResourceType "Sites" -Force
  ```
