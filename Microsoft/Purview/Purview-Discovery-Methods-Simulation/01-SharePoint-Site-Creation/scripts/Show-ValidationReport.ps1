...
Write-Host "🔍 Step 1: Locate Most Recent Validation Report" -ForegroundColor Green
...
try {
    ...
    if (-not (Test-Path $reportsFolder)) {
        ...
        Write-Host "   💡 Run Verify-SiteCreation.ps1 first to generate validation reports" -ForegroundColor Yellow
        exit 1
    }
    ...
    if ([string]::IsNullOrEmpty($ReportPath)) {
        ...
        if ($reportFiles.Count -eq 0) {
            Write-Host "   ❌ No validation report files found in: $reportsFolder" -ForegroundColor Red
            Write-Host "   💡 Run Verify-SiteCreation.ps1 first to generate validation reports" -ForegroundColor Yellow
            exit 1
        }
        ...
        if (-not (Test-Path $ReportPath)) {
            Write-Host "   ❌ Specified report file not found: $ReportPath" -ForegroundColor Red
            exit 1
        }
    }
    
    Write-Host "   ✅ Found report: $(Split-Path $ReportPath -Leaf)" -ForegroundColor Green
    Write-Host "   📅 Created: $((Get-Item $ReportPath).LastWriteTime.ToString('yyyy-MM-dd HH:mm:ss'))" -ForegroundColor Cyan
    
} catch {
    Write-Host "   ❌ Error locating validation report - $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
...
Write-Host "`n🔍 Step 2: Load Report Data" -ForegroundColor Green
Write-Host "============================" -ForegroundColor Green

try {
    $reportData = Get-Content $ReportPath -Raw | ConvertFrom-Json
    Write-Host "   ✅ Report data loaded successfully" -ForegroundColor Green
    
} catch {
    Write-Host "   ❌ Error loading report data - $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
...
Write-Host "`n📊 Site Validation Report Summary" -ForegroundColor Cyan
...
Write-Host "`n📋 Site-Level Validation Results" -ForegroundColor Cyan
Write-Host "=================================" -ForegroundColor Cyan
...
if ($fullyValidated.Count -gt 0) {
    Write-Host "`n✅ Fully Validated Sites:" -ForegroundColor Green
...
if ($partiallyValidated.Count -gt 0) {
    Write-Host "`n⚠️  Partially Validated Sites:" -ForegroundColor Yellow
...
if ($failed.Count -gt 0) {
    Write-Host "`n❌ Failed Validation:" -ForegroundColor Red
    foreach ($siteName in $failed) {
        Write-Host "   • $siteName" -ForegroundColor Red
    }
}
...
Write-Host "`n🔒 External Sharing Status" -ForegroundColor Cyan
Write-Host "==========================" -ForegroundColor Cyan
...
if ($externalSharingDisabledCount -eq $totalSitesCount) {
    Write-Host "   ✅ External sharing disabled on all $totalSitesCount sites" -ForegroundColor Green
} else {
    Write-Host "   ⚠️  External sharing status:" -ForegroundColor Yellow
...
Write-Host "`n✅ Report displayed successfully" -ForegroundColor Green
exit 0
