...
Write-Host "\n🔍 Step 1: Load Data for Analysis" -ForegroundColor Green
...
if ($DetailedAnalysisCsvPath -and (Test-Path $DetailedAnalysisCsvPath)) {
    Write-Host "📊 Loading detailed analysis CSV from Lab 05b..." -ForegroundColor Cyan
    ...
    Write-Host "   ✅ Loaded $($eDiscoveryData.Count) detection rows" -ForegroundColor Green
    $usingDetailedCsv = $true
} else {
    # Fall back to raw eDiscovery export
    if (-not $eDiscoveryExportPath) {
        Write-Host "📋 Searching for most recent Items_0 CSV file..." -ForegroundColor Cyan
        ...
        if ($items0Csv) {
            ...
            Write-Host "   ✅ Found: $(Split-Path $eDiscoveryExportPath -Leaf)" -ForegroundColor Green
        } else {
            Write-Host "   ❌ No Items_0*.csv files found in reports folder" -ForegroundColor Red
            Write-Host "   💡 Please provide -eDiscoveryExportPath parameter" -ForegroundColor Yellow
            exit 1
        }
    }
    
    if (-not (Test-Path $eDiscoveryExportPath)) {
        Write-Host "   ❌ File not found: $eDiscoveryExportPath" -ForegroundColor Red
        exit 1
    }
    
    Write-Host "📄 Loading raw eDiscovery export..." -ForegroundColor Cyan
    $eDiscoveryData = Import-Csv -Path $eDiscoveryExportPath
    Write-Host "   ✅ Loaded $($eDiscoveryData.Count) items" -ForegroundColor Green
}
...
Write-Host "`n📊 Step 2: Extract Unique GUIDs from Data" -ForegroundColor Green
Write-Host "===========================================" -ForegroundColor Green
...
if ($usingDetailedCsv) {
    ...
    Write-Host "📋 Extracting GUIDs from SIT_Type column..." -ForegroundColor Cyan
    ...
    Write-Host "📋 Extracting GUIDs from raw SIT data..." -ForegroundColor Cyan
...
Write-Host "   ✅ Found $($allGuids.Count) unique GUIDs" -ForegroundColor Green
Write-Host "   📊 Total GUID occurrences: $($allGuids.Values | Measure-Object -Sum | Select-Object -ExpandProperty Sum)" -ForegroundColor Cyan

# =============================================================================
# Step 3: Query Tenant for Available SITs
# =============================================================================

Write-Host "`n🔐 Step 3: Query Tenant SIT Definitions" -ForegroundColor Green
Write-Host "=========================================" -ForegroundColor Green
...
try {
    ...
    Write-Host "🔄 Retrieving all SIT definitions from tenant..." -ForegroundColor Cyan
    $tenantSits = Get-DlpSensitiveInformationType
    
    Write-Host "   ✅ Retrieved $($tenantSits.Count) SIT definitions" -ForegroundColor Green
    ...
    Write-Host "   ⚠️ Could not retrieve tenant SIT definitions: $_" -ForegroundColor Yellow
    Write-Host "   💡 Some analysis features will be limited" -ForegroundColor Yellow
    $tenantSitMap = @{}
}
...
Write-Host "`n📚 Step 4: Load Existing Centralized Mapping File" -ForegroundColor Green
Write-Host "===================================================" -ForegroundColor Green
...
if (Test-Path $centralizedMappingPath) {
    Write-Host "📄 Loading centralized mapping file..." -ForegroundColor Cyan
    ...
    try {
        ...
        if ($mappingContent.sitMappings) {
            ...
            Write-Host "   ✅ Loaded $($existingMappings.Count) existing GUID mappings" -ForegroundColor Green
        } else {
            Write-Host "   ⚠️ No sitMappings section found in JSON file" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "   ⚠️ Error loading mapping file: $_" -ForegroundColor Yellow
        Write-Host "   💡 Continuing without existing mappings" -ForegroundColor Yellow
    }
} else {
    Write-Host "   💡 No existing mapping file found at: $centralizedMappingPath" -ForegroundColor Yellow
    Write-Host "   📋 Will create new mapping file with discovered mappings" -ForegroundColor Cyan
}
...
Write-Host "`n❓ Step 5: Identify Unmapped GUIDs" -ForegroundColor Green
Write-Host "====================================" -ForegroundColor Green
...
Write-Host "   ✅ Mapped: $($mappedGuids.Count) GUIDs" -ForegroundColor Green
if ($mappedGuids.Count -gt 0) {
    Write-Host "      📊 Sources:" -ForegroundColor Cyan
    $mappedGuids | Group-Object Source | ForEach-Object {
        Write-Host "         • $($_.Name): $($_.Count) GUIDs" -ForegroundColor Gray
    }
}
Write-Host "   ❌ Unmapped: $($unmappedGuids.Count) GUIDs" -ForegroundColor Red

if ($unmappedGuids.Count -eq 0) {
    Write-Host "`n🎉 Success! All GUIDs are mapped to friendly names!" -ForegroundColor Green
    Write-Host "   💡 No unmapped GUIDs found - all SITs resolved via tenant or mapping file" -ForegroundColor Cyan
    exit 0
}

Write-Host "`n   🔍 Unmapped GUIDs requiring investigation:" -ForegroundColor Yellow
$unmappedGuids | Sort-Object -Property Occurrences -Descending | ForEach-Object {
    Write-Host "      • $($_.GUID) - $($_.Occurrences) occurrences" -ForegroundColor Yellow
}
...
Write-Host "`n🔬 Step 6: Analyze Context for Unmapped GUIDs" -ForegroundColor Green
Write-Host "===============================================" -ForegroundColor Green

$contextAnalysis = @()

foreach ($unmappedGuid in $unmappedGuids) {
    Write-Host "`n   📋 Analyzing GUID: $($unmappedGuid.GUID)" -ForegroundColor Cyan
    ...
    Write-Host "      📊 Occurrences: $($unmappedGuid.Occurrences)" -ForegroundColor White
    Write-Host "      📁 File Pattern: $(if ($fileNamePattern) { $fileNamePattern } else { 'None detected' })" -ForegroundColor White
    Write-Host "      🔗 Co-occurring SITs:" -ForegroundColor White
    
    if ($coOccurringSits) {
        foreach ($coSit in $coOccurringSits) {
            Write-Host "         • $($coSit.Name) ($($coSit.CoOccurrences) times)" -ForegroundColor Gray
        }
    } else {
    ...
    if ($ShowDetailedAnalysis) {
        ...
        if ($usingDetailedCsv) {
            ...
            $sampleFiles | Select-Object -First 3 | ForEach-Object {
                Write-Host "         • $($_.'File name')" -ForegroundColor Gray
            }
        }
    }
...
Write-Host "\n💡 Step 7: Recommendations" -ForegroundColor Green
Write-Host "===========================" -ForegroundColor Green

Write-Host "`n   📋 Summary of Unmapped GUIDs:" -ForegroundColor Cyan
...
Write-Host "`n   📝 To Update JSON Mapping:" -ForegroundColor Yellow
...
Write-Host "      • Purview Portal: https://compliance.microsoft.com/classifiers?viewid=sensitiveTypes" -ForegroundColor White
Write-Host "      • Microsoft Learn SIT Definitions: https://learn.microsoft.com/en-us/purview/sit-sensitive-information-type-entity-definitions" -ForegroundColor White

# =============================================================================
# Step 8: Optional - Generate JSON Update Template
# =============================================================================

Write-Host "\n📄 Step 8: Generate JSON Update Template" -ForegroundColor Green
Write-Host "==========================================" -ForegroundColor Green
...
Write-Host "`n🔄 Step 9: Interactive SIT Mapping Update" -ForegroundColor Green
Write-Host "==========================================" -ForegroundColor Green

$newMappings = @{}

foreach ($unmapped in $unmappedGuids | Sort-Object -Property Occurrences -Descending) {
    Write-Host "`n📋 Unmapped GUID: $($unmapped.GUID)" -ForegroundColor Cyan
    Write-Host "   📊 Occurrences: $($unmapped.Occurrences)" -ForegroundColor White
    
    # Try to find in tenant (one more time with fresh query)
    if ($tenantSitMap.Count -gt 0) {
        $tenantMatch = $tenantSitMap[$unmapped.GUID]
        if ($tenantMatch) {
            Write-Host "   ✅ Found in tenant: $tenantMatch" -ForegroundColor Green
            ...
            if ($response -eq 'Y' -or $response -eq 'y') {
                ...
                Write-Host "   ✅ Mapping queued for addition" -ForegroundColor Green
            } else {
    ...
    Write-Host "   ❌ Not found in tenant SIT definitions" -ForegroundColor Red
    ...
    if ($suggestion) {
        Write-Host "   💡 Suggested mapping based on $suggestionReason" -ForegroundColor Cyan
        ...
        if ($choice -eq '1') {
            ...
            if ($confirm -eq 'Y' -or $confirm -eq 'y') {
                ...
                Write-Host "   ⏭️  Cancelled" -ForegroundColor Gray
            }
        } elseif ($choice -eq '2') {
            ...
            if ($friendlyName -and $friendlyName.Trim() -ne '') {
                ...
                Write-Host "   ⏭️  Skipped (no name provided)" -ForegroundColor Gray
            }
        } else {
            Write-Host "   ⏭️  Skipped" -ForegroundColor Gray
        }
        ...
        Write-Host "   💭 No automatic suggestion available based on context" -ForegroundColor Gray
...
if ($newMappings.Count -gt 0) {
    Write-Host "`n💾 Step 9: Update JSON Mapping File" -ForegroundColor Green
    Write-Host "====================================" -ForegroundColor Green
    
    Write-Host "   📋 New mappings to add: $($newMappings.Count)" -ForegroundColor Cyan
    ...
    try {
        ...
        Write-Host "`n   ✅ Successfully updated: $OutputJsonPath" -ForegroundColor Green
        Write-Host "   📊 Total mappings in file: $($jsonContent.sitMappings.PSObject.Properties.Count)" -ForegroundColor Cyan
        ...
        Write-Host "`n   ❌ Error updating JSON file: $_" -ForegroundColor Red
        Write-Host "   💡 Manual update required - see template file: $templatePath" -ForegroundColor Yellow
    }
} else {
    Write-Host "`n⏭️  No new mappings added" -ForegroundColor Gray
}

Write-Host "`n✅ Analysis complete!" -ForegroundColor Green
Write-Host "📊 Analyzed $($unmappedGuids.Count) unmapped GUIDs" -ForegroundColor Cyan
