...
function Connect-PurviewServices {
    ...
    # Determine authentication mode: use interactive unless certificate auth params are provided
    $useInteractive = $Interactive -or (-not $ClientId -and -not $Thumbprint -and -not $Tenant)
            ...
            switch ($service) {
                ...
                "Exchange" {
                    ...
                    if ($useInteractive) {
                        ...
                        Connect-ExchangeOnline -AppId $ClientId -CertificateThumbprint $Thumbprint -Organization $Tenant -ErrorAction Stop
                ...
                "ComplianceCenter" {
                    # ComplianceCenter requires Connect-IPPSSession for DLP cmdlets
                    # (Get-DlpCompliancePolicy, Get-DlpComplianceRule, etc.)
                    # Connect-ExchangeOnline does NOT provide access to these cmdlets
                    ...
                    # Use both methods to ensure it takes effect
                    ...
                    if ($useInteractive) {
                        ...
                        if ($null -ne $existingSession) {
                            ...
                            Write-Host "   📋 Please sign in when prompted..." -ForegroundColor Cyan
                            # Use Connect-IPPSSession for Security & Compliance PowerShell
                            # This is REQUIRED for DLP cmdlets (Get-DlpCompliancePolicy, etc.)
                            # Try modern auth first (no Basic auth needed), fall back to RPS if needed
                    ...
                    # Note: Exchange is automatically available when connected to Compliance Center
                    $Script:ConnectedServices["Exchange"] = $true
                }
            }
        } catch {
