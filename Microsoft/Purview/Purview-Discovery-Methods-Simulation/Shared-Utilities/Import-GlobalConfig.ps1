<#
.SYNOPSIS
    Imports and validates the global configuration file for Purview Discovery Methods Simulation.
...
if ($config.Simulation.ScaleLevel -notin $validScaleLevels) {
    ...
    # Derive TotalDocuments from ScaleLevel
    $scaleDocumentMapping = @{
        "Small"  = 1000
        "Medium" = 5000
        "Large"  = 20000
    }
    $derivedTotalDocuments = $scaleDocumentMapping[$config.Simulation.ScaleLevel]
    
    # Ensure DocumentGeneration section exists and set TotalDocuments
    if (-not $config.PSObject.Properties.Name.Contains("DocumentGeneration")) {
        $config | Add-Member -NotePropertyName "DocumentGeneration" -NotePropertyValue ([PSCustomObject]@{})
    }
    if ($config.DocumentGeneration.PSObject.Properties.Name.Contains("TotalDocuments")) {
        $config.DocumentGeneration.TotalDocuments = $derivedTotalDocuments
    } else {
        $config.DocumentGeneration | Add-Member -NotePropertyName "TotalDocuments" -NotePropertyValue $derivedTotalDocuments
    }
    Write-Host "   ✅ TotalDocuments derived from ScaleLevel: $derivedTotalDocuments" -ForegroundColor Green
}
...
# =============================================================================
# Script Execution: Call the function with script-level parameters
# =============================================================================

# Call the function and return the result
Import-GlobalConfig -GlobalConfigPath $GlobalConfigPath -ValidateOnly:$ValidateOnly