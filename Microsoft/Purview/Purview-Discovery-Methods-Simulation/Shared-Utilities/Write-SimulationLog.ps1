<#
.SYNOPSIS
    Centralized logging infrastructure for Purview Discovery Methods Simulation scripts.
...
if ($enableFileOutput -and -not [string]::IsNullOrWhiteSpace($logDirectory)) {
    try {
        # Resolve relative paths to absolute (relative to project root, not script location)
        $resolvedLogDir = $logDirectory
        if (-not [System.IO.Path]::IsPathRooted($logDirectory)) {
            # Get project root - use the script's location to find Shared-Utilities parent
            $scriptDir = $PSScriptRoot
            if ([string]::IsNullOrEmpty($scriptDir)) {
                # Fallback if PSScriptRoot is not available (e.g., when called via &)
                $scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Definition
            }
            $projectRoot = Split-Path $scriptDir -Parent
            $resolvedLogDir = Join-Path $projectRoot $logDirectory.TrimStart('./').TrimStart('.\\')
        }
        
        # Ensure log directory exists
        if (-not (Test-Path -Path $resolvedLogDir)) {
        ...
        $logFilePath = Join-Path $resolvedLogDir $logFileName
