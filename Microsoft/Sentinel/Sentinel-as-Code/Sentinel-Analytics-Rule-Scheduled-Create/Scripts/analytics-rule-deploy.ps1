<#
.SYNOPSIS
    Deploys or updates Microsoft Sentinel scheduled analytics rules with external KQL query processing and comprehensive validation.

.DESCRIPTION
    This script automates the deployment of Microsoft Sentinel scheduled analytics rules using a pipeline-driven 
...
.PARAMETER TemplateFilePath
    ...
    Required: Yes
    Type: String
    Example: "C:\Templates\scheduled-analytics-rule-payload.json"
...
.PARAMETER KQLFilePath
    ...
    "@scheduled-analytics-rule-query.kql", the pipeline must pass the path to that exact filename.
    This enforces consistency between template configuration and pipeline variables.
    
    Required: Yes
    Type: String
    Example: "$(Build.SourcesDirectory)/Templates/scheduled-analytics-rule-query.kql"
